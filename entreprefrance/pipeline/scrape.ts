...
async function main(source: "manual" | "auto" = "auto"): Promise<void> {
  ...
  console.log(`\nðŸ—ž  BEpaper Scraper â€” ${startedAt}\n`);
  ...
  if (enabledFeeds.length === 0) {
    console.log("âš ï¸  No RSS feeds enabled in config â€” skipping scrape.");
    return;
  }

  for (const feed of enabledFeeds) {
    console.log(`â³ Scraping ${feed.name}â€¦`);
    try {
      const result = await scrapeRssFeed(feed.url, feed.name);
      if (result.error) {
        console.error(`  âŒ ${feed.name}: ERROR â€” ${result.error}`);
      } else {
        console.log(`  âœ… ${feed.name}: ${result.articles.length} articles found`);
        allArticles.push(...result.articles);
      }
    } catch (err) {
      console.error(
        `  âŒ ${feed.name}: UNEXPECTED ERROR â€” ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
  ...
  if (duplicatesRemoved > 0) {
    console.log(`\nðŸ” Removed ${duplicatesRemoved} duplicate(s)`);
  }
  ...
  console.log(`\nâœ… Done. ${deduped.length} articles saved to ${OUTPUT_FILE}\n`);
}
