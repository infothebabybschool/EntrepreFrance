...
export interface PipelineConfig {
  ...
  posting: {
    mode: PostingMode;
    firstPostTime: string;    // "HH:MM" â€” used by interval, random, same-time
    intervalMinutes: number;  // used by interval
    randomMin: number;        // used by random (minutes)
    randomMax: number;        // used by random (minutes)
    specificTimes: string[];  // used by specific â€” one "HH:MM" per article
  };
  rssFeeds: RssFeed[];
  images: {
    ...
    costLog?: Record<string, number>; // YYYY-MM-DD â†’ number of AI images generated that day
...
// In-memory cache â€” updated by refreshConfigFromApi()
...
export async function refreshConfigFromApi(): Promise<void> {
  ...
  try {
    ...
    // Keep using cached/file config â€” silent fail
  }
}
