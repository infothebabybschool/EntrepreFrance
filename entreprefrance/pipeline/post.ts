...
export async function checkAndPost(): Promise<void> {
  if (isPosting) {
    logScheduler("poster", "Previous run still in progress â€” skipping this tick");
    return;
  }
...
async function _checkAndPost(): Promise<void> {
  logScheduler("poster", "Checking for articles to postâ€¦");

  if (!fs.existsSync(ARTICLES_FILE)) {
    logScheduler("poster", "No articles_ready.json found â€” skipping");
    return;
  }
  ...
  await syncLog("poster", `${due.length} article(s) due â€” starting postingâ€¦`);
  ...
  for (const article of due) {
    ...
    try {
      ...
      const msg = axios.isAxiosError(err)
        ...
        : String(err);
      logSchedulerError("poster", `First attempt failed: ${msg} â€” retrying in 30s`);
      await syncLog("poster", `âš ï¸ First attempt failed: ${msg} â€” retrying in 30s`, true);
      await sleep(30000);
      ...
      try {
        ...
        const retryMsg = axios.isAxiosError(retryErr)
          ...
          : String(retryErr);
        logSchedulerError("poster", `Retry failed: ${retryMsg} â€” marking as failed`);
        await syncLog("poster", `âŒ Retry failed: ${retryMsg} â€” marked as failed`, true);
        target.failed = true;
      }
    }
    ...
    if (succeeded) {
      ...
      logScheduler("poster", `âœ… Posted successfully at ${actualTime} UTC`);
      await syncLog("poster", `âœ… Posted: "${article.title.slice(0, 70)}" â€” published_at: ${article.scheduled_for}`);
    }
...
export async function checkAndPostBySlug(slug: string): Promise<void> {
  ...
  if (target.posted) {
    // Article was already auto-posted â€” sync cache so admin panel reflects the correct state
    logScheduler("poster", `Article already posted: ${slug} â€” syncing cache`);
    await syncReadyCache(fileData.generated_at, fileData.total, fileData.articles);
    return;
  }
  ...
  await syncReadyCache(fileData.generated_at, fileData.total, fileData.articles);
  logScheduler("poster", `âœ… Posted: ${slug}`);
  await syncLog("poster", `âœ… Posted: "${target.title.slice(0, 70)}" (${slug})`);
}
