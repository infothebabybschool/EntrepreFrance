...
function buildPrompt(count: number, parity: number, diversity: number, countries: string[]): string {
  ...
  const countryContext = countries.length > 0
    ...
    : `The journalists work for ${SITE_NAME} (${TAGLINE}). Names should reflect relevant backgrounds appropriate to the diversity setting.`;

  return `You are a creative director building a fictional team of journalists for ${SITE_NAME} (${TAGLINE}).
