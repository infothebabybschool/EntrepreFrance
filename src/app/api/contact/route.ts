...
export async function POST(req: NextRequest) {
  ...
  const to = process.env.CONTACT_EMAIL ?? `newsletter@${SITE_DOMAIN}`;
  const resend = getResend();

  if (resend) {
    try {
      await resend.emails.send({
        from: `${SITE_NAME} Contact <newsletter@${SITE_DOMAIN}>`,
        to,
        replyTo: email,
        subject: `[${SITE_NAME} Contact] ${subject ?? "Message"}`,
        html: `
          <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
            <div style="background: #0f1f3d; padding: 20px; text-align: center;">
              <h1 style="color: #fff; font-size: 22px; margin: 0;">${SITE_NAME} — Message reçu</h1>
            </div>
