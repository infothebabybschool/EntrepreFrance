...
import DeleteJournalistButton from "./DeleteJournalistButton";
import GenerateTeamPanel from "@/components/admin/GenerateTeamPanel";

export const dynamic = "force-dynamic";

// Rank order for org chart
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="font-serif text-2xl font-bold text-gray-900">Équipe de rédaction</h1>
        <Link
      ...
      {!journalists?.length ? (
        ...
        <>
          {/* Organigram — only shown when there are multiple ranks */}
          {hasOrgData && (
            <div className="mb-8">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      {j.job_title && (
                        <p className="text-xs text-gray-500">{j.job_title}</p>
                      )}
                      {!j.active && (
...
function OrgCard({ journalist }: { journalist: { id: string; name: string; photo_url: string | null; job_title: string | null } }) {
  return (
