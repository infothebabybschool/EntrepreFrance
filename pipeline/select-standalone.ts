...
const OUTPUT_FILE = path.join(__dirname, "data", "selected_articles.json");
...
async function run(): Promise<void> {
  ...
  if (mode === "refresh") {
    log("select-standalone", `Mode: refresh — selecting ${articlesPerDay} articles from scratch`);
    ...
    log("select-standalone", `✅ ${selected.length} articles selected`);
    ...
    if (remaining <= 0) {
      log("select-standalone", `Selection already full (${alreadySelected.length}/${articlesPerDay}) — nothing to add`);
      return;
    }

    const excludeUrls = alreadySelected.flatMap((a: any) => Array.isArray(a.source_urls) ? a.source_urls : [a.source_url].filter(Boolean));
    log("select-standalone", `Mode: add-more — selecting ${remaining} more (${alreadySelected.length} already selected)`);
    ...
    fs.writeFileSync(OUTPUT_FILE, JSON.stringify(output, null, 2), "utf-8");
    log("select-standalone", `✅ ${added.length} articles added — total ${merged.length}/${articlesPerDay}`);
  }
}
